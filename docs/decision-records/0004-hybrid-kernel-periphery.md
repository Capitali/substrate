...
Substrate is an *evolutionary* factory: its core loop is generate → test → select,
...
- a **compiled, deterministic kernel** (Rust) for the parts that must be
  ...
  the obedience guard;
- an **interpreted / data-driven / generated periphery** the factory mutates
...
- **Expresses two Soul principles directly:** "the model is not the factory" and
