...
Substrate is, by design, a long-running autonomous process with **unrestricted
...
- **The host and network the factory inhabits** — it must not become a vector
...
| Malicious/compromised input (signals, LLM responses) | Prompt/observation injection steering the factory toward harm |
...
| The factory's own drift | Optimizing a proxy in a way that harms the served (Law II / reward-hacking) |
...
- **No exfiltration** — the factory sends no telemetry and phones nothing home; the
